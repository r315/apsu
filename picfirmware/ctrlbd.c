		...
		//data received	
			switch(i2cTransfer.status){
				...
				case WRITE_DATA_L:
					...
					doFunc = functions[i2cTransfer.datah >> 4]; // set what to do with 
					i2cTransfer.status = IDLE;					// received data
					doFunc((int*)&i2cTransfer.datal);			// work data
					break;						 
				case READ_DATA_L:
